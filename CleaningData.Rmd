...
Codebook describing the variables and dat and any transformations that were performed for cleaning up the data.
...
MeanType has values ending with .mean.  Eg: tBodyAccMag.mean.  We need to separate the .mean from the value.

SeparateData <- GatherData %>%
    separate(MeanType, into = c("Feature","Mean"), sep = "\\.") %>%
    separate(StdType, into = c("StdName", "Std"), sep = "\\.")

The above command seprates the value in to tBodyAccMag under Feature name and StdName column and “mean” under “Mean” , std under Std column.  
...
Following code removes those redundant columns Mean and Std which has values “mean” and “std”.   StdName and Feature are redundant. So we can drop StdName as well.
...
We select MeanValue ( which is the mean of all features ) and StdValue ( which is the standard deviation of all features ) and create two new columns, Mean and StandardDeviation containing the mean  of all features grouped by Subject/Activity/Feature. And Standard Deviation grouped by Subject/Activity/Feature.

The final output looks likes this:-


> names(TidyData)
[1] "Subject"           "Activity"          "Feature"           "Mean"              "StandardDeviation"
> head(TidyData)
Source: local data frame [6 x 5]
Groups: Subject, Activity

  Subject Activity              Feature       Mean StandardDeviation
